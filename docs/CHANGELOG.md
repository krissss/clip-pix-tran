# Changelog

## [Unreleased]

## [0.6.2] - 2026-07-17

### Fixed

- 修复首次设置开机自启失败

## [0.6.1] - 2026-07-05

### Added

- 添加应用品牌标识
- 更换新的菜单栏图标

## [0.6.0] - 2026-07-02

### Added

- 支持截图步骤标注
- pix: 在截图框选中添加二维码识别

### Changed

- 优化区域截图工具条显示卡顿
- 优化图片历史内存占用
- 移除 gitnexus

### Fixed

- 强化自动更新兜底唤醒
- 收紧窗口的桌面可见范围

## [0.5.0] - 2026-06-18

### Added

- 添加 Pix 截图 OCR 功能
- 支持手动截长图
- 支持双击预览图片

### Changed

- 移除 OpenAI 翻译与发音支持
- 移除翻译硬编码 fallback

### Fixed

- 修复历史列表选中样式与点击区域
- 修复系统翻译首次不可用误判

## [0.4.0] - 2026-06-11

### Added

- 支持开机启动设置
- 支持应用内语言切换

### Fixed

- validate release signing certificate
- clarify release certificate validation
- support legacy release certificates
- 精简界面冗余文案
- 优化翻译面板与预览布局

## [0.3.0] - 2026-06-10

### Added

- 支持跨版本更新日志展示

### Fixed

- 修复 release workflow 认证
- 优先响应 Debug 版全局快捷键
- 统一更新提示 UI 风格
- 禁用 Debug 构建更新检查

## [0.2.0] - 2026-06-09

### Added

- 支持菜单栏直接打开设置，添加debug标识
- 添加首次启动引导

### Fixed

- 截图操作栏右对齐
- 修复截图无权限时缺少提示
- 优化设置页表单对齐

## [0.1.1] - 2026-06-07

### Fixed

- 显示版本并本地化更新提示

## [0.1.0] - 2026-06-07

### Added

- 完成 Clip、Pix、Tran MVP 功能
- 增强 Clip 剪贴板历史与快速面板
- 增强 Tran 划词翻译和多 provider 展示
- 增强 Pix 区域截图标注体验
- 统一控制面板 UI 风格
- 增强 Tran 划词翻译体验
- 增强 Tran 多 provider 翻译体验
- 增强 Tran 发音 provider 体验
- Pix 支持桌面贴图
- Pix 支持自动识别截图区域
- 菜单栏支持启动静默和 Dock 偏好
- 清空历史增加二次确认
- Pix 支持录屏和 GIF 导出
- Pix 优化录屏和历史预览交互
- 优化控制面板快捷入口和预览交互
- 添加 AI skills
- 添加自动更新和发版流程

### Fixed

- 修复 Pix 区域截图菜单栏面板保留问题

[Unreleased]: https://github.com/krissss/clip-pix-tran/compare/v0.6.2...HEAD
[0.6.2]: https://github.com/krissss/clip-pix-tran/compare/v0.6.1...v0.6.2
[0.6.1]: https://github.com/krissss/clip-pix-tran/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/krissss/clip-pix-tran/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/krissss/clip-pix-tran/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/krissss/clip-pix-tran/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/krissss/clip-pix-tran/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/krissss/clip-pix-tran/compare/v0.1.1...v0.2.0
[0.1.1]: https://github.com/krissss/clip-pix-tran/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/krissss/clip-pix-tran/commits/v0.1.0
